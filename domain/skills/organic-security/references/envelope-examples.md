...
```yaml
status: ok
executive_summary: "Threat model complete. 2 MINOR findings (no CRITICAL). Recommend folding security_requirements into the next Task Brief."
mode: threat-model
artifacts: []
security_lens:
  status: findings
  findings:
    - { id: "F-1", severity: MINOR, file: "src/auth/session.ts", line: 34, claim: "Session tokens have no explicit expiry policy stated in scope." }
security_requirements:
...
```yaml
status: warning
executive_summary: "Threat model complete. 1 CRITICAL finding: hardcoded API key in the described approach. Override required to proceed."
mode: threat-model
artifacts: []
security_lens:
  status: findings
  findings:
    - { id: "F-1", severity: CRITICAL, file: "src/config/keys.ts", line: 12, claim: "Hardcoded API key literal in source." }
security_requirements:
