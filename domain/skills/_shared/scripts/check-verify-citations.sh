...
#
# Usage: check-verify-citations.sh <verification-report.md> [repo-root] [tasks-md]
#        tasks-md defaults to the tasks.md sitting next to the report.
...
usage() { echo "usage: $0 <verification-report.md> [repo-root] [tasks-md]" >&2; exit 2; }
[ "$#" -ge 1 ] || usage
report="$1"
root="${2:-.}"
tasks_md="${3:-}"
...
echo "citation-audit: $checked test refs checked, $checklist_checked checklist refs checked, $skipped checklist refs skipped (no tasks.md), $violations unresolved"
[ "$violations" -eq 0 ]
