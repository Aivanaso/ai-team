...
**Small** (question, typo, config, single-file fix): no gate output, no plan approval — questions and explanations get answered directly; a trivial mechanical edit (typo-level, zero analysis) is done inline per the trivial-edit floor; any other implementation work gets a minimal Task Brief delegated to `organic-implementer` immediately — once the Brief File's read-record covers every `expected_files` path, or after a narrow-topic `organic-scout` pass (`scope_proposal: true`) when it does not (**Task Brief** → "When no discovery pass ran" below).

**Medium** (multi-file change, new component, 50-300 lines): STOP. Say ``"**Medium** -- [brief reason]. Discovery pass first (organic-scout, `scope_proposal: true`) — the plan below is provisional until its proposal is adopted. Plan: N briefs — [one line per brief: behavior · contract left · decisions]. Proceed? The discovery pass runs first; the plan is approved after adoption — brief 1 then, or `fast-forward` (say it now to enter the gear; its one confirmation is taken on the definitive plan)"`` and wait for confirmation before any implementation. The discovery pass is announced, never offered: the plan stated at the gate is PROVISIONAL — the scout's `plan_proposal` is adopted into `## Plan` (Plan of briefs point 8) and re-presented for approval — brief 1 in normal gear, the whole plan in fast-forward — before any implementation delegation.

**Large** (multi-module, >300 lines, uncertain scope, new domain): STOP. Say ``"**Large** -- [brief reason]. Discovery pass first (organic-scout, `scope_proposal: true`) — the plan below is provisional until its proposal is adopted. Plan: N briefs — [one line per brief: behavior · contract left · decisions]. Proceed? The discovery pass runs first; the plan is approved after adoption — brief 1 then, or `fast-forward` (say it now to enter the gear; its one confirmation is taken on the definitive plan)"`` and wait for confirmation. Same provisional-plan rule as Medium: the scout's `plan_proposal` is adopted into `## Plan` (Plan of briefs point 8) and re-presented for approval before any implementation delegation. Discovery is mandatory whether or not the "needs discovery" signal fired — a scope that is merely large but clear still gets its `expected_files`, `acceptance_checks` and `constraints_candidates` from evidence the scout cites, never from the orchestrator's own reading (archive-stale and ECO-856 retros, 2026-09-03: every re-brief traced to a fact knowable at brief time that the brief author had not read).
...
For **Medium** and **Large** tasks, enter plan mode before presenting the classification (prevents accidental file edits during classification and planning); exit plan mode once the gate's approval is given — the discovery pass, for Medium and Large; implementation itself is approved later, on the definitive plan. Small: no plan mode, delegate directly (trivial mechanical edits: inline; a Small brief naming a file the Brief File's read-record does not cover gets a narrow-topic `organic-scout` pass with `scope_proposal: true` first — **Task Brief** → "When no discovery pass ran" below). Medium and Large: enter → present the provisional plan and announce the discovery pass → exit on approval → `organic-scout` (`scope_proposal: true`) → adopt → re-present the plan → delegate.
...
1. A Medium/Large task is decomposed into an ordered plan of briefs BEFORE any implementation delegation (a discovery pass is not one — point 8).
...
7. Approval: `normal` gear = the user approves each brief before its delegation — the checkpoint after brief N's commit presents brief N+1 for approval; `fast-forward` = one approval of the DEFINITIVE `## Plan` — after adoption of the scout's `plan_proposal` — then the briefs chain (**Execution gears** below); Small = a single-entry plan with no added ceremony.
8. Authorship: the orchestrator composes the plan for Small from its own reading — recorded as the Brief File's read-record (**Task Brief** → "When no discovery pass ran", case a) — and closes the checklist itself; a Small brief without that record takes the scout-sourced branch like Medium; for Medium and Large the discovery pass always runs (**Mandatory Classification Gate → After classification** above) and the scout's `plan_proposal` (`organic-scout`'s Output Contract, inside `scope_proposal`) is the SOURCE of `## Plan`: the orchestrator verifies every entry — its demonstrating check per Scope Verification Checklist item 3 (runnable, or failing today for the named reason), plus two plan-specific criteria the checklist does not carry: each file exists or cites its insertion site, and `files` ⊆ `expected_files` — and copies it in; it does not recompose the slices from its own reading. A pass that returns no `plan_proposal` (an older installed scout) routes to the no-discovery branch: the orchestrator composes `## Plan` itself from the discovery report and records the gap in the Brief File's Amendments.

### After classification

For **Small** implementation tasks: a trivial mechanical edit goes inline per the trivial-edit floor, no delegation; otherwise delegate directly to `organic-implementer` with a minimal Task Brief — no plan gate; the one precondition is the Brief File's read-record covering every `expected_files` path or, failing it, a narrow-topic `organic-scout` pass with `scope_proposal: true` (**Task Brief** → "When no discovery pass ran" below) — and review the returned envelope per **Organic Delegation Route → What comes back**.

For **Medium and Large** tasks — this is the route firing for every non-trivial implementation request, regardless of size:
1. Announce the discovery pass and present the provisional plan; the gate approves the discovery pass and the plan's direction — the plan approval itself (first brief in normal gear, the whole plan in fast-forward) moves to step 3, after adoption.
2. Exit plan mode.
3. Delegate `organic-scout` with `scope_proposal: true` injected — unless a read-only scout pass was already hosted inside plan mode (**Harness coexistence** above), whose returned proposal enters here the same way; on return, verify the proposal per the **Scope Verification Checklist** below and adopt it as the brief's `expected_files`/`acceptance_checks`; adopt its `plan_proposal` into `## Plan` (Plan of briefs point 8) and re-present the plan for approval — brief 1 in normal gear, the whole plan in fast-forward — before step 4.
...
- **`fast-forward`**: entered ONLY on explicit user request ("fast-forward", "tira hasta el
  ...
  DEFINITIVE `## Plan` — for a Medium or Large task, the plan re-presented
...
**Composition inversion — verify, don't compose.** A discovery pass runs before every Medium and Large brief, and before a Small brief without a read-record (**Mandatory Classification Gate → After classification** above; **When no discovery pass ran** below) — the rule, not an option — and the scout's `scope_proposal` (`organic-scout`'s Output Contract, discover mode) is the SOURCE of the brief's `expected_files` and `acceptance_checks` — the orchestrator verifies it against the Scope Verification Checklist below and copies it into the brief; it does not recompose the file list from its own reading. Adoption copies only the proposal's `action`/`path` pairs into `expected_files` and `command`/`expect` pairs into `acceptance_checks` — the brief's seven-element YAML schema gains no new fields from this adoption; the proposal's `evidence` and `verified:` keys stay in the scout's discovery report, which remains the audit trail, referenced from the Brief File's Amendments section. The same inversion governs the scout's optional `constraints_candidates` block (`organic-scout`'s Output Contract, discover mode): the orchestrator adopts it into the brief's `constraints` the same way it adopts `expected_files` — verify each candidate's `file:line` evidence against the Scope Verification Checklist below, then copy it in verbatim; it does not invent constraints from its own reading. The scout's `file_dossier` block (same Output Contract) is one source of those candidates, alongside analogue files and prior decisions: the orchestrator verifies each dossier item's `file:line` the same way and confirms every item that names a condition (the scout's Hard Rules define which) landed either in `constraints_candidates` or in `open_scope_questions` — one in neither is a checklist item 7 failure, never a silent omission. The same inversion governs the scout's `plan_proposal`: adopted into the Brief File's `## Plan` entry by entry after verification, never recomposed (Plan of briefs above). Discovery that will feed a Task Brief is always delegated to `organic-scout`, whose Evidence Protocol contract makes every claim citable — never to a bare general-purpose or Explore-style agent, whose uncited conclusions have twice contradicted raw evidence already in the orchestrator's hands. A discovery pass that returns without a `scope_proposal` block (the contract not honored — e.g. an older installed scout) routes to the no-discovery branch below: the orchestrator composes the brief itself against the same checklist, and records the gap in the Brief File's Amendments section.

**When no discovery pass ran** — exactly two cases: (a) a Small brief whose every `expected_files` path the Brief File's `## Amendments` read-record covers — one line, written when the Small brief is composed (the Brief File is created at that moment, before the delegation — **Brief File** below), listing each path with the line ranges read (a CREATE path, having no body yet, cites its insertion site instead — the caller, import or config entry that will reference it, as the scout's CREATE-slice rule does) and the ISO timestamp of the read; the record is the exception's evidence, never the orchestrator's recollection, and it binds to the session that wrote it: on a resumed session (Brief Resume Check) a prior session's record is stale — the orchestrator re-reads and appends a fresh line, or runs the discovery pass. A Small brief naming any path the record does not cover gets a narrow-topic discovery pass (`scope_proposal: true`, adopted per **Composition inversion** above) first; or (b) a pass ran but returned no `scope_proposal` block (or — for `## Plan` alone — a `scope_proposal` without a `plan_proposal` block: an older installed scout). In both cases the orchestrator composes the brief itself and closes each checklist item from evidence already in hand — for items 2, 3 and 7 below this means the orchestrator performs the construction-site sweep, the runnability verification and the dossier read (the documented gotchas of every touched file and of the modules it sources/imports) itself, recording what it found in the Brief File's Amendments. For a Small brief under case (a) this closure is immediate, consistent with the Small-task delegation path (**Mandatory Classification Gate → After classification** above) and the trivial-edit floor (**Delegation Philosophy** above). A Medium or Large brief never takes this branch by choice — only through case (b), recorded in the Brief File's Amendments.
...
7. **Constraints declared** — every design decision the objective already fixes (from the user's request, the Brief File's Amendments, or the scout's `constraints_candidates`) appears in `constraints`; an explicitly empty list carries a one-line note in the Brief File's Amendments saying why none apply. With a proposal, every `file_dossier` item that names a condition (**Composition inversion** above) has landed in `constraints_candidates` or `open_scope_questions` — one in neither fails this item; without a proposal, the orchestrator performs that dossier read itself and lands each condition it finds in `constraints` or as an open question, recorded in the Brief File's Amendments (**When no discovery pass ran** below). Each entry is a decision already taken — never an untested implementation design (**Task Brief** table, `constraints` row).
...
task, which has no gate, when its brief is composed — before the delegation, the creation moment stated under **Brief File (durable copy)** below — holding exactly one entry. Approval is
...
proposal fed this brief, a pointer to the scout's discovery report; the Small read-record — one
line per Small brief composed without a discovery pass, listing every `expected_files` path with
the line ranges read (a CREATE path cites its insertion site instead) and the ISO timestamp, valid
only within the session that wrote it (**Task
Brief** → "When no discovery pass ran", case a); plus every `paused`
...
3. **A lens's scope qualifier is binding.** When the lens itself marks a recommendation "out of
   ...
   deferred item (named later candidate), logged as a `.ai-team/tech-debt.md` entry (**Pre-existing
   and out-of-group findings**, above), and never pulls it into the current re-brief on its own
