...
| **Discovery skill** (`organic-scout`) | Mandatory pre-brief discovery (Medium/Large; Small without a read-record) — `scope_proposal`, `constraints_candidates`, `plan_proposal`; bootstrap `.ai-team/config.yaml` | Findings summary; `scope_proposal` (when requested); `config.yaml` | Read-only on application code; never writes application files. |
