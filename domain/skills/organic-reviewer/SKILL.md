...
- Sidecar dogfooding, every pass without exception (full and DELTA MODE): the `.json` sidecar is written THROUGH the same validator the orchestrator's Citation audit runs (`python3 skills/_shared/scripts/check-receipt.py receipt {sidecar} .`, Execution Step 7), and the report carries a `## Sidecar Self-Validation` section recording the command, exit code, and any VIOLATION fixed before return — symmetric to `organic-security`'s rule.
