...
- A verification re-run that contradicts the implementer's claimed `check_results` (a check the envelope reported `pass` now fails, or vice versa) is itself a CRITICAL correctness finding, never silently reconciled. The claimed rows arrive as the injected `check_results` field (orchestrator-protocol.md → Critical Context Forwarding); when it is absent, this reconciliation cannot run — note the gap in `risks` rather than asserting agreement.
...
- TDD evidence audit, gated on `strict_tdd` being injected (present only when the STRICT TDD MODE directive was sent to the implementer): (a) a `tdd_cycles[].test` naming a test absent from the candidate (`group_files` / `git diff HEAD`) is a MAJOR finding, `evidence: read`, `trigger` = the cycle; (b) a `red.digest` that is a zero-work output or an unrelated error, or that does not name which of the two valid reasons applies — a red counts only when the test ran and failed on its own assertion, or failed to compile/load because it calls production code that does not exist yet; a zero-work output (`No tests found`, `0 suites`, a runner or configuration error) is never a red — it is a test that did not run (`_shared/evidence-protocol.md` → Rule 7 item 3) — is a MAJOR finding, `evidence: read`, `trigger` = the digest; (c) a behavior visible in the diff — the same enumeration the `decisions_taken` rule above uses, plus a new public function or branch — with no covering cycle is a MAJOR finding, `evidence: read`, `trigger` citing the site; (d) `strict_tdd` injected but neither `tdd_cycles` nor `tdd_not_applicable` forwarded, or a `tdd_not_applicable` reason the diff contradicts, is a MAJOR finding, `evidence: read`, `trigger` = the injected `strict_tdd` flag plus the absent or contradicted field. All four are policy deviations (MAJOR at most, mirroring the constraint-contradiction precedent above) — never CRITICAL by itself. Composes with the evidence-cap and severity rules already stated above — every clause names its `trigger`, so the cap never downgrades them. -- because a directive with no auditable trace is indistinguishable from no directive at all.
- TDD mutation probe, gated on `strict_tdd` being injected, OPTIONAL and never mandatory per cycle, never a reason to widen the pass: the reviewer MAY break the covered production change on a scratch copy (never the working tree) and re-run that cycle's test; if it stays green, record a CRITICAL finding, `evidence: executed`, `trigger` = the mutation plus the green result. -- because a test that cannot fail when the behavior it claims to cover is broken was never testing that behavior.
- Receipt dogfooding, every pass without exception (full and DELTA MODE): the report is written THROUGH the same validator the orchestrator's Citation audit runs (`python3 skills/_shared/scripts/check-receipt.py receipt {report_destination} .`, Execution Step 7), and it carries a `## Receipt Self-Validation` section recording the command, exit code, and any VIOLATION fixed before return — symmetric to `organic-security`'s rule.
- One report, one block: the receipt lives in the report's own single fenced ```json block, and every other JSON excerpt anywhere in that report is fenced as ```text or indented, never as ```json — a second block makes the whole report a structural VIOLATION the validator rejects at exit 1. -- because the gate reads the file's one block, so a quoted excerpt in a finding would silently compete with the receipt for it.
...
| `strict_tdd` is injected and a `tdd_cycles` entry names a test absent from the candidate, carries a zero-work, unrelated, or reason-less `red.digest`, an uncovered diff-visible behavior has no cycle, or `tdd_not_applicable`/`tdd_cycles` is missing or contradicted | Record a MAJOR finding, `evidence: read`, per the TDD evidence audit Hard Rule — never CRITICAL by itself. |
| `strict_tdd` is injected and the reviewer runs the OPTIONAL mutation probe (breaks a covered production change on a scratch copy, re-runs that cycle's test) | Test stays green → record a CRITICAL finding, `evidence: executed`, `trigger` = the mutation + the green result. Never mandatory, never a reason to widen the pass. |
...
| A declared check — an `acceptance_checks` command, a `config.yaml`-declared build/lint command, or a `review_gates` entry — cannot be executed in this environment at all (missing tool, unreachable command) | Omit it from `verification`; note the gap in `risks` — never fabricate `pass` or `fail`. If EVERY declared check is unrunnable and `verification` ends up empty, set `verification_omitted_reason` naming that fact — the receipt gate rejects an unexplained empty list. |
...
1. Read `_shared/context-protocol.md` (startup) and `_shared/persistence-contract.md` (write rules — loaded per common-rules Principle 5; this skill writes no `.ai-team/` artifact by default). Validate injected context: `project_root`, `group_id`, `group_files`, `tier`, `tier_reason`, the implementer's result envelope (when forwarded), `decisions_taken` (optional — present only when the implementer's envelope reported ≥1 entry; its absence is normal, not degradation), `strict_tdd` plus `tdd_cycles` (with `tdd_cycles_omitted` / `tdd_not_applicable`) — optional, present only when the STRICT TDD MODE directive was sent to the implementer; absence is normal, never degradation, `report_destination` (always injected for review-plane passes per Critical Context Forwarding — treat absence as degradation: report `context_resolution: fallback` and flag it in `risks`), and — for DELTA MODE — `prior_report` plus its delta scope. Report `context_resolution` honestly.
...
7. When `report_destination` is injected, write the report per `references/report-format.md` there (create its parent directory if absent), resolved relative to `project_root` — use the Delta Report Variant in DELTA MODE. That one file ends with a `## Receipt` heading followed by a single fenced ```json block carrying the exact Review Receipt object composed at Step 6 (same field names, no additions); nothing else is written beside it. Self-check the report before returning: `python3 skills/_shared/scripts/check-receipt.py receipt {report_destination} .`; fix the block and re-run on any printed `VIOLATION` line before returning — the orchestrator's own Citation audit re-runs this same validator on this same file as its BLOCKING gate, reading only that block and never the prose around it. `report_destination` is always injected for review-plane passes; in the degraded case where it is absent, the envelope's Review Receipt is the only record, nothing exists on disk to validate, and the blocking Citation audit cannot fire — flag that in `risks`.
8. Return the envelope per Output Contract.

## Output Contract

Writes ONE report at the injected `report_destination` (resolved relative to `project_root`),
whose final fenced ```json block carries the Review Receipt object verbatim — mandatory from the
orchestrator's side for every review-plane delegation (`orchestrator-protocol.md` → Critical
Context Forwarding); optional only from this skill's own write step, i.e. it writes nothing when
no destination is injected. No fixed path, no separate `.ai-team/` artifact, no second file
beside the report. Returns:
...
executive_summary: "..."
group_id: "<brief-slug>"
artifacts: []                   # only when report_destination was written this run — the one report file, receipt block included
tier: 1 | 2
