...
2. `retro`: read the injected `brief_file` in full — frontmatter (`status`, `mode`, dates), `## Plan`, every `## Task Brief` block, `## Phases`, `## Cost Ledger`, `## Amendments`, `## Close`, `## Ledger`. Missing or unreadable → the matching blocked gate. Read the ledger/close/`plan` figures for Metrics FROM the `## Ledger` block — the single fenced ```json block the file ends with, the machine-validated source (`_shared/scripts/check-receipt.py ledger`, `orchestrator-protocol.md` → Task Brief → "Brief File structural check"); a Brief File written before that block existed may instead have a separate same-slug JSON file next to it, and the figures come from there; with neither, fall back to reproducing the `.md` file's `## Cost Ledger` table verbatim, unchanged from before.
