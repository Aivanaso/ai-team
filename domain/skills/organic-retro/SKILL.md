...
Cost Ledger, Phases, and Amendments plus any injected review-report paths; when its own
...
2. `retro`: read the injected `brief_file` in full — frontmatter (`status`, `mode`, dates), every `## Task Brief` block, `## Phases`, `## Cost Ledger`, `## Amendments`, `## Close`. Missing or unreadable → the matching blocked gate.
