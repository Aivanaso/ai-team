...
This mode is triggered automatically by the orchestrator when `.ai-team/config.yaml` is missing — the user never invokes it directly.
...
```yaml
status: ok
executive_summary: "Detected {stack summary}. Generated config.yaml."
artifacts:
  - name: "config"
