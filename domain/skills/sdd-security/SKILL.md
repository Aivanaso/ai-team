...
| `mode: code-audit` with an incomplete SDD set (only one or two of `tasks_path`/`change_branch`/`base_branch`) | `status: blocked`, naming the missing field(s) — never run the diff scan with an unbound branch value. |
...
1. Read `_shared/context-protocol.md` (startup), `_shared/persistence-contract.md` (write rules). Validate injected context: `change_name`, `change_dir`, `mode`, `project_root`, plus mode-dependent inputs — SDD mode: `proposal_path`, `tasks_path`, `change_branch`, `base_branch`; non-SDD mode: `group_files`, `report_destination`. Their absence in non-SDD mode is expected and is NOT a `context_resolution: fallback` signal. Precedence: if **all three** SDD inputs (`tasks_path`/`change_branch`/`base_branch`) are present, the SDD diff scan wins regardless of what else is injected. If **some but not all** are present, `status: blocked` naming the missing field(s) — never run the diff scan with an unbound branch value (see Decision Gates). If **none** are present, non-SDD mode applies per `group_files`/`report_destination`.
...
2b. **(non-SDD)** Resolve each path in the injected `group_files` relative to the injected `project_root`; read each in full, plus up to 10 1-hop callers — no branch diff is computed. Scope any git inspection (e.g. `git status`) with `-C {project_root}`.
3. Read `config.yaml` (non-SDD mode: read it from `{project_root}`, the audited repo). If `test_commands.security:` exists, run it and capture output. If absent: log "Dependency auditor: not configured (skipped)".
...
5. Write the report per [references/audit-report-template.md](references/audit-report-template.md) — to `{change_dir}/audit-report.md` in SDD mode, or to the injected `report_destination`, resolved relative to `project_root`, in non-SDD mode (create its parent directory if absent). All 6 category sections MUST be present ("No findings" if clean).
