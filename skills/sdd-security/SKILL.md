...
- If `mode: threat-model` → proceed to Section "Process: Mode threat-model" (Steps 8.1–8.5)
...
## Process: Mode `threat-model` (Steps 8.1–8.5)
...
If `security_touchpoints` is an empty list: return `status: ok`, `verdict: no-findings`, `findings: []`, noting "no touchpoints triggered — security gate skipped" in the executive summary.
...
| `category` | One of the 5 vulnerability categories (code-audit) or one of the 9 touchpoints (threat-model) |
...
2. **Empty `security_touchpoints` in threat-model** → return `status: ok`, `verdict: no-findings`, `findings: []`, note "no touchpoints triggered — security gate skipped" (REQ-SECURITY-002 Scenario 2.2).
